...
function NewAdminsList({cameraID}) {
    ...
    const fetchAdmins = async () => {
        setLoading(true);
        api.get(`/api/cameras/${cameraID}/get-new-admins/?page=${page}&page_size=${pageSize.value}`)
            .then(response => {
    ...
    const addAdmins = async (ids) => {
        setLoading(true);
        api.post(`/api/cameras/${cameraID}/add-admins/`, {admins: ids}
        ).then(() => fetchAdmins()
        ).catch(error => {
            if(error.response) setError(error.response.data.detail || "Internal Server Error");
            else if(error.request) setError("Cannot connect to the server.");
    ...
    const handleConfirmConfirmation = () => {
        setShowConfirmation(false);
        addAdmins(admins.map(admin => admin.id));
    }

    return (
        <div className="flex flex-col text-center justify-center px-5 w-2/5 my-10 mx-auto">
            <h1 className="mb-5 text-black dark:text-white font-bold text-3xl">Admins to assign</h1>
            ...
            {admins.length > 0 &&
                <button className="confirm-button w-[200px] px-4 py-2 mb-4 mx-auto rounded-full text-white text-xl transition"
                        ...
                        {admins.map((item, index) => (
                            ...
                            transition-transform hover:-translate-y-0.5 hover:shadow-xl hover:bg-white/20"
                              ...
                              <span className="flex-1 font-medium text-black dark:text-white break-words">
                                ...
                                <div className="flex">
                                    <button className="confirm-button px-3 py-1 rounded-full text-sm ml-2 text-white"
                                            onClick={() => {addAdmins([item.id])}}
                                    >Assign</button>
                                </div>
                            </li>
                        ))}
                    </ol> : <h2 className="text-2xl">No admins to assign</h2>}
            </div>
            ...
            {showConfirmation && <ConfirmWindow message="assign all admins" onConfirm={handleConfirmConfirmation} onClose={handleCloseConfirmation}></ConfirmWindow>}
        </div>
    );
}

export default NewAdminsList;